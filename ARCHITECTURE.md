...
This table is about capability, not speed. Millrace is measured against Hangfire and WorkflowCore in
[docs/benchmarks.md](docs/benchmarks.md), method and caveats included (§11.37).
...
34. **Public API documentation is complete, and `CS1591` is now a build error (2026-07-26).** Closes [#99](https://github.com/inferenceailab/Millrace/issues/99). §11.31 turned documentation generation on, suppressed the missing-comment warning, and named deleting that suppression as the deliverable. It is deleted.
    - **The count in the issue was wrong, and how it was wrong is the lesson.** 164 was the core package at one moment. Across the solution it was 404 — and 52 of those were not ours at all: the SDK's default `**/*.cs` glob had been reaching into `node_modules` and compiling node-gyp's `Find-VisualStudio.cs` into the Angular package, shipping eleven public COM interop types. A suppression added for one reason had been hiding a defect of an entirely different kind, invisible because the warning it produced was already off. Re-measure before trusting a number that justifies a suppression.
    - **Three shapes, chosen by what the member is.** Contract types and value types got written prose, because the reasoning had nowhere else to live. Implementations got `<inheritdoc />`, because the contract is documented once on the interface and a second copy could only drift from it. Read-model projections got `<inheritdoc cref="..."/>` pointing at the record field they project, which the compiler resolves to a full documentation id — one copy of the text, and the projection cannot drift from the record.
    - **What a comment is for.** Not to restate the member's name. Where a member's meaning is obvious, the comment says what is *not*: when a field is null, who writes it, which guarantee the member does not carry. `GetJobAsync` returning an unfenced snapshot, `IStorageNotifier` signals being droppable, `Retry.Fixed` setting `MaxDelay` equal to its delay — none of that is inferable from a signature, and all of it was folklore.
    - **`Millrace.Storage.Verification` keeps the suppression, scoped to itself.** Around 110 of its public members are `[Fact]` and `[Theory]` methods, public because xUnit requires it rather than because anything calls them, and their names already are the specification. What a provider author implements or calls — `IStorageHarness`, each suite's abstract `CreateHarnessAsync`, the fixture helpers — is documented. Scoping rather than leaving it global is the point: everywhere else, a new undocumented public member now fails the build.
    - **It is an error, not a warning**, because `TreatWarningsAsErrors` is on. That makes this a ratchet rather than a milestone: the state cannot quietly regress the way it arrived.

35. **A completed inner saga declares whether an outer unwind undoes it (2026-07-27).** Implements [#77](https://github.com/inferenceailab/Millrace/issues/77) alongside §11.29, which settled the other half.
    - **§11.29 answered inner *failure* and left inner *success* open.** It settled that a failing inner saga unwinds itself and then propagates outward. It said nothing about the case that turns up just as often: the inner saga succeeded, the outer failed later, and the question is whether work committed inside the inner one is now undone. The engine answered it by accident — a completed inner saga was never recorded as a step of the outer, so an outer unwind silently skipped it.
    - **Neither answer is right for every saga, so the author declares it.** §11.29's own argument says an outer saga's promise is broken if a nested construct lets it proceed past work that did not happen; the mirror of that is an outer saga whose unwind leaves committed inner work in place. But the opposite case is just as real — an inner saga that took payment through a provider with no reversal, or that notified a third party, is *final*, and replaying its compensations would be a second wrong rather than an undo.
    - **So `Saga` inside a saga takes a `NestedSagaPolicy` and there is no default.** A nested saga is a deliberate construct, and the choice changes what the outer saga guarantees. Requiring it puts the decision at the one place with the context to make it. A default can be added later if that proves tiresome; it cannot be taken away.
    - **`Unwind` keeps the inner saga's record after it commits**, so the outer has something to replay — the inner's completed steps stay on the cursor with `Compensating` false, and the outer records the inner *saga* as one of its own steps. Unwinding that entry restarts the inner's compensations in reverse rather than running one activity, which is what keeps innermost-first (§11.29) true in this direction too.
    - **`Keep` drops the record on commit**, exactly as an un-nested saga does today. The outer unwinds only its own direct steps.
    - **The parent is derived, not stored.** A nested saga's own node carries the outer's `SagaId`, so the enclosing saga is `Node(innerSagaId).SagaId` — no new cursor field, and no way for the nesting to drift from the graph it came from. The persisted cursor is unchanged, so instances in flight are unaffected.

36. **The Blazor UI package carries the .NET WebAssembly runtime, and that is the price (2026-07-27).** Implements [#47](https://github.com/inferenceailab/Millrace/issues/47). §11.23 chose native Blazor on rendering-quality grounds and never priced the packaging; measuring it first is what this records.
    - **Roughly 6.4 MB against 151–212 KB for the React and Angular bundles.** Measured, not estimated: a stock publish is 25.9 MB, 17.2 MB once the `.gz`/`.br` duplicates are dropped, 8.9 MB without the template's Bootstrap, and 6.4 MB with invariant globalization. What remains is `dotnet.native.wasm` at 2.9 MB and `System.Private.CoreLib` at 1.6 MB — a Blazor app ships a .NET runtime, and no amount of care makes it not.
    - **Accepted rather than engineered around.** The package is opt-in, a consumer installs it only if they want Blazor, and an operations dashboard is not a public-facing page where first-paint bytes decide anything. The alternative was worse: the `wasm-tools` workload would trim it meaningfully and would become a third thing a contributor must install before the build works at all. The Node floor already demonstrated what one missing prerequisite costs, and it was a version bump rather than a workload.
    - **So the build stays `dotnet` and `npm`, and the size is documented instead of hidden.** Invariant globalization is on and the compressed duplicates are not embedded — serving them would need content negotiation the middleware does not do, so they are 8 MB of files nothing can ever read.
    - **Parity is checked against the assemblies, not a script.** §11.22 reads a UI's bundle and asserts every contract route appears in it. A Blazor app's routes are in `_framework/*.wasm`, not its JavaScript, and .NET string literals are UTF-16 — so the check encodes each fragment UTF-16LE and scans the embedded assemblies. Same guarantee and same one-directional claim as before: a bundle that never mentions an endpoint certainly does not call it.

37. **Benchmarks are a harness in the repository, not a BenchmarkDotNet suite (2026-07-27).** Implements [#49](https://github.com/inferenceailab/Millrace/issues/49). The numbers, and the caveats that come with them, are in [docs/benchmarks.md](docs/benchmarks.md); what is recorded here is why the method is what it is.
    - **BenchmarkDotNet is the wrong instrument for this.** Its model — many iterations of a cheap deterministic call, outlier removal, per-invocation attribution — assumes the thing measured is a method. What is measured here is a worker pool, a connection pool and PostgreSQL under concurrent load, where the interesting behaviour only exists in aggregate and a single "operation" is a round trip someone else's process has to serve. Running that in a tight iteration loop measures none of it. So: a console harness that seeds a backlog, drains it, and reports wall-clock throughput and enqueue-to-execute percentiles.
    - **Fairness is the whole product, so the rules are published rather than implied.** One PostgreSQL server with a database per system, recreated per run; identical worker concurrency; an identical empty job body; setup — DDL, EF Core migrations, JIT — outside the measured window; median of three with every repeat and the spread published.
    - **Where a knob had to move, it moved in the comparand's favour.** Millrace runs on shipped defaults in every published row. Hangfire gets long polling and a 200 ms queue poll; WorkflowCore gets its concurrency raised from 4. A benchmark that tunes the system under test and leaves the comparands on defaults is measuring the author's willingness to read two documentation pages.
    - **Two tunings, both published.** *Matched* puts every system on the same polling floor, which is the only way a latency number describes a design rather than a default. *Default* leaves each system as it ships, which is what an evaluator meets on day one. Either one alone is a different kind of misleading.
    - **Millrace is measured twice per scenario with nothing changed between the two rows**, because its defaults already sit at the matched floor. That pair is a repeatability control: the gap between two identical configurations is this harness's noise floor, and a reader can see it without being asked to trust the spread column.
    - **The metric is completions divided by time from a standing start, not first-completion to last.** The narrower window was tried first and is wrong for anything whose intermediate work is not counted: a three-step workflow only records its final step, so the window would open once the pipeline was already two-thirds full and report a rate the engine never sustained.
    - **The harness is in the solution so it compiles in CI, and it never runs there.** Benchmarks that stop building are worse than none, but a wall-clock measurement on a shared runner is noise with a number attached. `Hangfire` and `WorkflowCore` are therefore in `Directory.Packages.props` under a label saying they are benchmark-only, and nothing that ships may reference them.

38. **A UI is not shipped until it has been opened in a browser (2026-07-28).** Implements [#122](https://github.com/inferenceailab/Millrace/issues/122). The Blazor UI reached `main` in [#120](https://github.com/inferenceailab/Millrace/pull/120) having never been run: it compiled, the §11.22 parity check passed, 58 dashboard tests passed, and the first time anyone loaded it — building the multi-view layout this issue asked for — it rendered nothing. Three separate defects, none of which any existing check could see.
    - **It could not read the contract.** The client used default `JsonSerializerOptions`, so every string enum the server writes (§11.24, the fix for the *opposite* bug) failed to deserialize and every list view died on its first row. The parity check passed throughout, because a route literal being present in an assembly says nothing about whether a response can be parsed.
    - **The trim-safe converter is the generic one.** `JsonStringEnumConverter` is a factory that builds converters by reflection; a published Blazor app is trimmed, the factory finds nothing it can construct, and enums silently fall back to numbers. `JsonStringEnumConverter<T>` per enum is what survives, which means the list has to be *complete* — four enums are reachable from the dashboard's responses, and a missing one fails only on the view that reads it. `BlazorWireFormatTests` walks the contract and pins the set rather than trusting anyone to remember.
    - **Blazor swallows fragment navigation.** Hash routing is what lets one prebuilt bundle work at any mount prefix (§11.23), but Blazor intercepts same-origin anchor clicks and navigates with `history.pushState` — measured as `pushState 1, hashchange 0`, with `LocationChanged` silent as well. A tab click changed the URL and raised nothing at all. So internal links go through `RouteLink`/`Navigator`, which navigate explicitly and write the URL themselves; the JS listener is left for back, forward, and a hand-edited fragment.
    - **`dotnet publish -o` does not clean, and Blazor fingerprints by content hash.** Every build therefore *added* an assembly beside the last one, and the package embedded all of them — eight copies of the app assembly had accumulated. The bundle is emptied before publishing now. §11.36's 6.4 MB was measured on a clean tree and stands, but it would have drifted upward silently.
    - **And the mount root never worked, for any UI.** All three reference their assets relatively, so serving the entry document at `{prefix}/ui` as well as `{prefix}/ui/` made the browser resolve every asset against the prefix instead of the UI directory. The document arrived and the page stayed blank. It now redirects — and the test that had asserted a 200 there, and passed while the page was broken, asserts the redirect.
    - **The lesson generalises past Blazor:** every check the repository had was static. Parity reads bytes for a substring, packaging asserts exported types, wire-format tests cover the TypeScript clients whose UIs someone had actually opened. None of them execute the thing. A rendering test is the gap (see [#126](https://github.com/inferenceailab/Millrace/issues/126)); until there is one, "I opened it" is a required step and not a nicety.

39. **The documentation site is docfx, and the API reference is generated from the XML comments (2026-07-28).** Closes [#48](https://github.com/inferenceailab/Millrace/issues/48), the last issue in the 1.0 milestone. The generator was the open decision; the content was never the hard part.
    - **docfx, because §11.34 already paid for it.** Every public member outside the conformance kit is documented, and `CS1591` is a build error keeping it that way — 649 documented members that nothing published. docfx is the only candidate that turns that enforced cost into a shipped artifact; Astro Starlight and mkdocs-material are better-looking and cannot generate .NET API reference at all, which would have left "read the source" as the answer to every API question. Statiq can do both and asks for materially more assembly with a much smaller community behind it.
    - **It also adds no toolchain.** docfx is a `dotnet tool`, pinned in `.config/dotnet-tools.json`, so the docs build needs the SDK this repository already requires. The Node-based generators would have added a second Node toolchain — permitted, since a docs site is not a published package, but §11.36 is on record about what one more prerequisite costs.
    - **Metadata generation passes `-p:SkipUiBuild=true`, so the docs build needs no Node**, and the workflow deliberately installs none. That is a check, not a convenience: if anything ever reintroduces the dependency, the docs job fails rather than quietly growing one.
    - **`--warningsAsErrors`, for the same reason `CS1574` is not suppressed** (§11.31). A broken `<xref:>` is a dead link in shipped documentation. Worth recording because the symptom does not name the cause: docfx prints *"Build succeeded with warning"* and exits non-zero, so the text says one thing and the exit code says another.
    - **A link checker, because docfx does not check the links it does not own.** It validates cross-references and API links; an ordinary `[jobs](jobs.md)` pointing at a renamed file renders as a link to nowhere and builds clean. `scripts/check-docs-links.ps1` resolves all 8,850 internal links against the rendered output. It is PowerShell rather than Node so the docs job keeps its no-Node claim, and it was verified by breaking a link on purpose and watching it exit 1 — the failure path is the only part of a checker worth testing.
    - **Built on pull requests, deployed only from `main`.** `release.yml`'s actions run only on a tag, so CI never exercises them and every release is their first real test — a blind spot this half did not close. Publishing docs from a workflow nobody runs until merge day would have repeated it.
    - **§11.38 applied to itself, and immediately collected.** The site built with zero warnings, all 741 API items exported, every cross-reference resolving — and the logo rendered at 69 pixels square, overlapping the brand text, because the modern template injects it client-side as `<img id="logo">` and sizes it from that id, so a class selector loses on specificity. Nothing static could see it. The browser saw it in one screenshot.

40. **One job executes the UIs in a browser; everything else about them stays static (2026-07-28).** Closes [#126](https://github.com/inferenceailab/Millrace/issues/126), which §11.38 asked for. The gap it fills is narrow and was expensive: every check this repository had read the bundles without running them, so a UI that rendered nothing passed all of them.
    - **The tests are written against what the three UIs share, not against any one framework's DOM.** That turned out to be more than expected: the hash routes are a contract (`#/jobs`, `#/jobs/{id}`, `#/recurring`, `#/instances`, `#/signals`), the management buttons carry the same labels, and every list view renders a Queue column. Asserting on framework internals would have produced three suites wearing a trench coat, and would rot the first time any UI was restyled.
    - **The assertion is that seeded data appears on the page.** Finding the probe queue name proves the bundle loaded, executed, called the API and *deserialized the response* — the four things that were all broken in #120, of which a 200 on the document proves none. The management test goes further and asserts on the job's state in **storage** after the click, because what is being checked is that a button reaches an endpoint, not that a page says it did.
    - **Kestrel on port 0, not `TestServer`.** The rest of the dashboard tests use an in-memory server with no socket, which a browser cannot reach. The bound port is read back from `IServerAddressesFeature` rather than guessed, so hosts can run in parallel.
    - **Strictness defaults to `CI`, exactly as the provider suites do.** `MILLRACE_REQUIRE_BROWSER` overrides; otherwise every CI job is strict unless it opts out in the workflow file. The inverse — opt in per job — leaves the next job someone adds free to skip the suite and still report success, which is the precise failure mode this suite exists to prevent. The main `build` matrix opts out visibly, because the browser lives in its own job.
    - **Its own project and its own job.** `Millrace.Dashboard.BrowserTests` keeps a downloaded browser out of the fast dashboard tests, and the CI job cannot pass `-p:SkipUiBuild=true` — it serves the bundles to a real browser, so skipping their build would leave it testing three empty packages and passing.
    - **Verified by a negative control before being believed.** The first green run took four seconds for nine browser tests, which was not plausible; a temporary test asserting that a *bogus* string is absent while the real one is present proved the locators genuinely inspect the DOM. The suite's own skip and strict paths were checked the same way — no browser and not required skips at exit 0, no browser and required fails at exit 1. Per §11.39, a checker that has never failed is not a checker.
    - **What it still does not cover**, deliberately: one browser engine (Chromium), no visual regression, and no assertion about layout. It answers "does it render and can it act", which is the question that was unanswered, rather than "does it look right".

41. **1.0 makes the contract promise (2026-07-28).** The storage contract and the v1 REST contract are stable: they do not break within 1.x. Every previous version number was deliberately conservative — §11.31 refused to bump it when the release pipeline landed, on the grounds that finishing the work and making the promise are separate acts — so this entry records the promise being made rather than the work being finished.
    - **What is now frozen.** `IJobStorage`, `IWorkflowStorage`, `IMonitoringStorage` and the records they exchange; the `/api/v1` surface the dashboard serves and the three UIs consume. A provider that passes the conformance kit today passes it against 1.x, and a client written against `v1` keeps working. Additive change is still change: new members on those interfaces break implementors, so they wait for 2.0 or arrive as separate interfaces the way §11.14 put monitoring in its own.
    - **What is deliberately *not* frozen.** Everything behind them — the engine, the workers, the scheduler, the providers' SQL, and the shape of the UIs. The conformance kit itself is a test suite rather than a contract, and it is expected to grow: a new fact there is a bug found, not a breaking change.
    - **`v1.0.0-rc.1` came first, and earned its keep.** It exercised the publishing path — untouched since its actions were upgraded — and proved the OIDC exchange, the nine-package push and the prerelease inference all still worked, at the cost of a version number rather than at the cost of the 1.0 tag.
    - **The README ships inside every package** (`Directory.Build.targets` packs the repository root one), so "Status: alpha" would have been embedded in nine 1.0.0 packages permanently — nuget.org has no delete. The documentation changes therefore landed *before* the tag rather than after it, which is the ordering any future version bump needs.
    - **Two things were knowingly carried into 1.0.** `Millrace.Storage.Verification` still suppresses `CS1591` for its `[Fact]` methods (§11.34), which is the package a community provider author reads. And the storage contract has twice grown from being built on — clause 7 in §11.16, compensation recovery in §11.30 — so a third provider might yet find a gap; SQLite is the cheapest way to find out, and after 1.0 that discovery is more expensive than before it. Both were weighed and accepted rather than overlooked.

42. **The repository's settings are part of the supply chain (2026-07-28).** Hardening done after 1.0 shipped, because publishing now depends on them: nuget.org's Trusted Publishing policy (§11.33) names this repository and the `release.yml` file, so **whoever can change that file or create a tag can publish as `inferenceailab`**. Until this, neither was protected.
    - **A ruleset can be active and enforce nothing.** `main` had a ruleset named "main", marked active, carrying deletion, non-fast-forward and pull-request rules — with an *empty* ref-name condition, so it matched no branches at all. `GET /repos/{owner}/{repo}/rules/branches/main` returned an empty list and `main` was fully open. The settings UI shows a green, plausible-looking rule; only the effective-rules endpoint tells the truth. This is §11.38's lesson in a place with no compiler: **check what applies to the branch, not what the rule says.**
    - **Release tags are immutable, with no bypass.** The tag *is* the version (§11.31) and nuget.org has no delete, so a moved or deleted `v*` is the one git operation that cannot be undone anywhere. Creation is still unrestricted; only deletion and update are blocked. No bypass actor, deliberately: the realistic threat is an accidental `git push --delete`, and an owner bypass would disable the guardrail precisely against the most likely cause.
    - **`main` keeps an admin bypass, and that is not a contradiction.** Anyone with admin can delete the ruleset outright, so a bypass costs almost nothing against a compromised owner while preventing a genuine deadlock: required status checks plus no bypass means a broken CI configuration cannot be fixed, because the fix cannot merge. Tags have no such deadlock, which is why they have no bypass.
    - **Pull requests are required with *zero* approvals.** A one-maintainer project cannot approve its own pull request, so requiring a review would have blocked every merge. Zero approvals still forces the branch through CI and leaves a diff to read, which is the part that was doing the work.
    - **A dependency was fixed by override rather than by the tool's advice.** `npm audit fix --force` proposed *downgrading* `@angular/cli` from 22.0.8 to 21.0.4 to escape a moderate path-traversal advisory in a transitive dev dependency. An `overrides` entry pinning the patched version keeps the toolchain current instead, and the browser suite (§11.40) is what confirms the forced major bump did not break the bundle it builds.
    - **Two settings the API accepted and ignored.** Secret-scanning validity checks and non-provider patterns both return `200 OK` and stay `disabled`; they need a tier this repository does not have. Recorded because a write that silently does nothing is worse than one that fails — the same shape as the ruleset above.
    - **Actions are an allowlist, not a marketplace.** `allowed_actions` is `selected`, with GitHub-owned actions permitted and **`verified_allowed` off** — the whole verified marketplace is a wider trust boundary than a package publisher needs, and exactly one third-party action is in use. `NuGet/login@*` is allowed by explicit pattern rather than by relying on a verified status that cannot be confirmed from the API. Adding any new third-party action is now a deliberate settings change, which is the point.
    - **The allowlist was verified against the action nothing else exercises.** CI passing proves only the `actions/*` half; `NuGet/login` runs solely from a tag, so a wrong pattern would have surfaced as a failed *release*. A throwaway workflow on a branch referenced the action with no `id-token` permission — so it could obtain no publishing token and publish nothing — and the log showed `Download action repository 'NuGet/login@v1'`, which a policy block prevents before any download. Branch deleted, never merged. The general point: when a control guards a path CI never runs, build the smallest thing that runs that path.
    - **Every action is pinned to a full commit SHA**, with the version in a trailing comment. The allowlist answers *which* actions may run; it does not stop `actions/checkout@v7` being re-pointed at different code tomorrow, because a major-version tag is mutable by design and the whole ecosystem's convention is to follow it. A SHA is the only reference that names the code rather than a label for it — which is the same argument §11.31 makes for the release tag being the version, applied in the other direction.
    - **The cost is that upgrades stop being invisible, which is also the benefit.** A pinned action never moves on its own, so a security fix upstream does not arrive until someone bumps it. `.github/dependabot.yml` grouping the `github-actions` ecosystem monthly is what closes that, and Dependabot reads the trailing version comment — so the comments are load-bearing, not decoration.
