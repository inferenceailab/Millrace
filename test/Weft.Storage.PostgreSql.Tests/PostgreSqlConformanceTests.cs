...
namespace Weft.Storage.PostgreSql.Tests;

/// <summary>
/// One PostgreSQL for the whole test run: <c>WEFT_POSTGRES_CONNECTION</c> if set (CI-provided
...
internal static class PostgresTestDatabase
{
    ...
    {
        if (Environment.GetEnvironmentVariable("WEFT_POSTGRES_CONNECTION") is { Length: > 0 } external)
        {
            return external;
        }
...
internal sealed class PostgreSqlHarness : IStorageHarness
{
    ...
    public static async ValueTask<IStorageHarness> CreateAsync(TimeProvider time)
    {
        ...
        {
            Assert.Skip("PostgreSQL unavailable — start Docker or set WEFT_POSTGRES_CONNECTION.");
        }

        // A fresh schema per harness gives the required isolated, empty store cheaply.
        var schema = $"weft_{Guid.NewGuid():n}";
