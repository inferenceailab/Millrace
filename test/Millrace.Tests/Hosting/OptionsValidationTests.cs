...
namespace Millrace.Tests.Hosting;

/// <summary>Startup validation must fail fast on configurations that would misbehave silently.</summary>
public class OptionsValidationTests
{
    private static IHost BuildHost(Action<MillraceOptions> configure)
    {
        var builder = Host.CreateEmptyApplicationBuilder(new HostApplicationBuilderSettings());
        builder.Services.AddLogging();
        builder.Services.AddMillrace(w => w.UseInMemoryStorage().Configure(configure));
